...
def initCenters(X, dataIndex):
    centers = np.zeros([len(dataIndex), X.shape[1]])  # placeholders of centers
...
def computeDis(x, centers):
    dis = np.zeros(len(centers))  # placeholders of distances to all centers
...
def updateCen(new_center):
    centers = np.zeros_like(new_center['center'])  # placeholders of centers

    # get the position of each center by division between the
...
def computeError(X, assign, centers):
    error = 0   # placeholders of errors
